...
city_coordinates = {
    ...
    "Hubli": (15.3647, 75.1240)  # Added Hubli coordinates
}
...
def add_specific_clinics():
    clinics_data = [
        {
            "name": "Nova IVF Fertility",
            "cities": ["Bengaluru", "Mysore", "Mangalore"],
            "state": "Karnataka",
            "phone": "+91-1800-200-8555",
            "description": "India's leading fertility chain offering advanced IVF, ICSI, donor programs, and fertility preservation. Known for high success rates and expert embryology labs."
        },
        {
            "name": "Oasis Fertility",
            "cities": ["Bengaluru", "Hubli"],
            "state": "Karnataka",
            "phone": "+91-8880-000-909",
            "description": "Comprehensive fertility care with personalized treatment protocols, advanced technologies, and patient-focused support through every fertility stage."
        },
        {
            "name": "Iswarya Fertility Centre",
            "cities": ["Bengaluru", "Mysore"],
            "state": "Karnataka",
            "phone": "+91-9840-999-333",
            "description": "Well-known fertility care center offering IVF, IUI, fertility diagnostics, and affordable treatment options with expert gynecologists and embryologists."
        },
        {
            "name": "Gaudium IVF Centre",
            "cities": ["Bengaluru"],
            "state": "Karnataka",
            "phone": "+91-844-888-3334",
            "description": "Advanced fertility center offering IVF, ICSI, donor services, egg freezing, and fertility preservation with modern lab technology and experienced specialists."
        },
        {
            "name": "Indira IVF",
            "cities": ["Bengaluru", "Hubli", "Mysore", "Belgaum"],
            "state": "Karnataka",
            "phone": "+91-766-500-8787",
            "description": "One of India's largest IVF chains featuring state-of-the-art labs, experienced fertility doctors, cost-effective treatment plans, and strong success rates."
        }
    ]
    ...
    with app.app_context():
        ...
        for clinic_data in clinics_data:
            for city in clinic_data["cities"]:
                # Check if clinic already exists by name and city
                existing_clinic = Clinic.query.filter_by(name=clinic_data["name"], city=city).first()
                if existing_clinic:
                    print(f"Clinic '{clinic_data['name']}' in {city} already exists, skipping.")
                    continue
                ...
                # Create address
                address = f"Branch in {city}, {clinic_data['state']}"
                ...
                clinics_added += 1
                print(f"Added clinic: {clinic_data['name']} in {city} (lat: {lat:.4f}, lng: {lng:.4f})")

        db.session.commit()
        print(f"Successfully added {clinics_added} specific clinics.")

if __name__ == "__main__":
    add_specific_clinics()
